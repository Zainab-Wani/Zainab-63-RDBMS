...
use scott;
show tables;
select deptno,max(sal) as maxsal from emp group by deptno;/*using dept number we print the sum of salary of emp using deptid*/
select job,sum(sal),count(*) from emp group by job;/*using jobid sum of salary by job*/

/*multi row funtion and group  by clause using jobid and deptid to calculate sum of sal by job*/
select deptno,job,sum(sal) as salsum,count(*) as total
from emp
group by deptno,job
order by deptno;
